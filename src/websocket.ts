...
export function mergeWebSocketParams<TEvent extends WebSocketEventType>(
  ...
  if (typeof params === 'string') {
    ...
    if (!merged.token || !merged.coin || !merged.network) {
      throw new Error(
        'When using the string event form, token, coin, and network must be set in the client options.',
      );
    }
  } else {
    merged = {
...
export class WebSocketConnectionManager {
  ...
  static getConnection<TCoin extends Coin, TEvent extends WebSocketEventType>(
    ...
    if (!this.connections.has(key)) {
      console.debug(
        `[WebSocketManager] Creating new connection for event '${options.event}' on ${options.coin}/${options.network} (token: ${!!options.token})`,
      );

      this.connections.set(key, new WebSocketConnection(options));
    } else {
      console.debug(
        `[WebSocketManager] Reusing existing connection for event '${options.event}' on ${options.coin}/${options.network} (token: ${!!options.token})`,
      );
    }
...
export default class WebSocketConnection<TCoin extends Coin, TEvent extends WebSocketEventType> {
  ...
  private connect() {
    ...
    this.websocketClient?.on('open', () => {
      console.debug(
        `[WebSocket] Connection opened for event '${this.options.event}' on ${this.options.coin}/${this.options.network}`,
      );
      ...
      this.websocketClient?.on('message', (buffer) => {
        ...
        console.debug(
          `[WebSocket] Message received for event '${this.options.event}' on ${this.options.coin}/${this.options.network}:`,
          message,
        );
        ...
        if (message && message.event === 'events limit reached') {
          ...
          throw new Error(
            'Blockcypher events limit reached. You may change your token or upgrade.',
          );
        }
    ...
    this.websocketClient?.on('close', () => {
      console.debug(
        `[WebSocket] Connection closed for event '${this.options.event}' on ${this.options.coin}/${this.options.network}`,
      );
    ...
    this.websocketClient?.on('error', (err) => {
      console.debug(
        `[WebSocket] Error for event '${this.options.event}' on ${this.options.coin}/${this.options.network}:`,
        err,
      );
