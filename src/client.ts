...
import {
  ...
  BlockMap,
  TransactionMap,
} from './typings/api-response-mappings';
...
> {
  ...
  private async request(
    path: string,
    options: Exclude<RequestOptions, '_currentRateLimitRetryAttempt'>,
  ): Promise<any> {
        ...
        if (response.status === 429) {
          ...
          if (options._currentRateLimitRetryAttempt <= (this.options.rateLimitRetries || 3)) {
            const retryAfter = response.headers.get('retry-after')
              ...
              : options._currentRateLimitRetryAttempt! * 1000;

            console.debug(
              `[API] Rate limit hit. Retrying after ${retryAfter}ms (attempt ${options._currentRateLimitRetryAttempt})`,
            );
  ...
  on<TEvent extends WebSocketEventType>(
    ...
    console.debug(
      `[Client] Registering callback for event '${merged.event}' on ${merged.coin}/${merged.network} (token: ${!!merged.token})`,
    );
  ...
  off<TEvent extends WebSocketEventType>(
    ...
    console.debug(
      `[Client] Removing callback for event '${merged.event}' on ${merged.coin}/${merged.network} (token: ${!!merged.token})`,
    );
  ...
  once<TEvent extends WebSocketEventType>(
    ...
    console.debug(
      `[Client] Registering once-callback for event '${merged.event}' on ${merged.coin}/${merged.network} (token: ${!!merged.token})`,
    );
